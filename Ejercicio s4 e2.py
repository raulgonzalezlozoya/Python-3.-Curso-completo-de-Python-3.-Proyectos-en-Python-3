# Ejercicio

# 1. Crear una variable "numero1" con el valor 5.

numero1 = 5

# 2. Crear una variable "numero2" con el valor 3.

numero2 = 3

# 3. Crear la variable "suma" con el valor de la suma de los numeros numero1 y numero2.

suma = numero1 + numero2

# 4. Convertir esta variable "suma" en una cadena de caracteres y llamarla "strSuma".

strSuma = str(suma)

# 5. Crear una variable resultado que contiene el texto "El resultado es " y la variable "strSuma".

resultado = "El resultado es " + strSuma
print(resultado)